      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <EcoTwinPreview
              interactive={false}
              initialScore={twin.score}
            />

            <GlassCard className="p-6">
              <div className="text-xs font-mono text-slate-400 uppercase mb-4">
                Your Profile
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <div className="text-xl mb-1">
                    {profile.onboarding?.diet === "vegan"
                      ? "🌱"
                      : profile.onboarding?.diet === "vegetarian"
                      ? "🥗"
                      : profile.onboarding?.diet === "heavy_meat"
                      ? "🥩"
                      : "🍽️"}
                  </div>

                  <div className="text-xs text-slate-400 capitalize">
                    {(profile.onboarding?.diet ?? "—").replace("_", " ")}
                  </div>
                </div>

                <div>
                  <div className="text-xl mb-1">
                    {profile.onboarding?.commute === "walk_cycle"
                      ? "🚴"
                      : profile.onboarding?.commute === "transit"
                      ? "🚌"
                      : profile.onboarding?.commute === "frequent_flights"
                      ? "✈️"
                      : "🚗"}
                  </div>

                  <div className="text-xs text-slate-400 capitalize">
                    {(profile.onboarding?.commute ?? "—").replace("_", " ")}
                  </div>
                </div>

                <div>
                  <div className="text-xl mb-1">
                    {profile.onboarding?.home === "small_apartment"
                      ? "🏢"
                      : profile.onboarding?.home === "shared_house"
                      ? "🏠"
                      : "🏡"}
                  </div>

                  <div className="text-xs text-slate-400 capitalize">
                    {(profile.onboarding?.home ?? "—").replace("_", " ")}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            ...
            <GlassCard className="p-6">
              ...
              <div className="mt-5 text-center">
                <span className="px-4 py-2 rounded-full bg-brand-emerald/10 border border-brand-emerald/20">
                  {badge}
                </span>
              </div>
          ...
          <div className="space-y-6">
            ...
            <GlassCard className="p-6">
              ...
              <p className="mt-3 text-sm text-slate-300">
                Earth's health restored by {earthHealth}%
              </p>
            </GlassCard>
            ...
            <GlassCard className="p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {achievements.map((achievement) => (
                  <span
                    key={achievement}
                    className="px-3 py-1 rounded-full bg-brand-emerald/10 border border-brand-emerald/20"
                  >
                    {achievement}
                  </span>
                ))}
            ...
            <GlassCard className="p-6">
              ...
              <input
                ...
                value={futureBoost}
                onChange={(e) =>
                  setFutureBoost(Number(e.target.value))
                }
                className="w-full"
              />

              <div className="text-3xl font-bold text-brand-emerald mt-5">
                {Math.min(
                  twin.score + futureBoost,
                  1000
                )}
              </div>
